  ...
  group("delivers", () {
    ...
    group('next', () {
      ...
      test('minutes interval starting from date', () {
        TZDateTime date = normalizedDate(
            TZDateTime(getLocation("Europe/Berlin"), 2020, 04, 01));
        var cronIterator = Cron().parse("*/5 * * * *", "Europe/Berlin");
        date = TZDateTime.from(date, getLocation("Europe/Berlin"));
    ...
    group('previous', () {
      ...
      test('days starting from date', () {
        ...
        date = TZDateTime.from(date, getLocation("Europe/Berlin"));
        expect(
            cronIterator.previous(), equals(date.subtract(Duration(days: 0))));
        expect(
            cronIterator.previous(), equals(date.subtract(Duration(days: 1))));
        expect(
            cronIterator.previous(), equals(date.subtract(Duration(days: 2))));
        expect(
            cronIterator.previous(), equals(date.subtract(Duration(days: 3))));
      });
